...
// Extra fields collected only when "also create user" is toggled on
...
const EMPTY_USER_EXTRA: UserExtraData = {
  phoneNumber: "",
  age: "",
  nid: "",
  gender: "Male",
...
const apiAddEmployee = (data: EmployeeFormData) =>
  authFetch(`${API_BASE}/Api/V1/Employee/Add`, {
    method: "POST",
    body: JSON.stringify({ ...data, baseSalary: Number(data.baseSalary) }),
  });

const apiRegisterUser = (data: EmployeeFormData & UserExtraData) =>
  authFetch(`${API_BASE}/Api/V1/users/register`, {
    method: "POST",
    body: JSON.stringify({
      name: `${data.employeeFirstName} ${data.employeeLastName}`.trim(),
...
// Fires employee + optional user request in parallel
const apiAddBoth = async (
  empData: EmployeeFormData,
  userExtra: UserExtraData | null,
): Promise<{ employee: any; user?: any; userError?: string }> => {
  if (!userExtra) {
    const employee = await apiAddEmployee(empData);
    return { employee };
  }
  // Fire both concurrently — employee failure throws, user failure is captured
  const [employee, userResult] = await Promise.allSettled([
    apiAddEmployee(empData),
    apiRegisterUser({ ...empData, ...userExtra }),
  ]);

  if (employee.status === "rejected")
    throw new Error((employee.reason as Error).message);

  return {
    employee: (employee as PromiseFulfilledResult<any>).value,
    user: userResult.status === "fulfilled" ? userResult.value : undefined,
    userError:
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${cls[status] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
    >
      <span
...
const EmployeeModal = ({
  ...
  saving: boolean;
  error?: string | null;
  userError?: string | null; // user creation partial failure
  onSave: (emp: EmployeeFormData, userExtra: UserExtraData | null) => void;
  onClose: () => void;
}) => {
  const [form, setForm] = useState<EmployeeFormData>(initial);
  const [userExtra, setUserExtra] = useState<UserExtraData>(EMPTY_USER_EXTRA);
  const [createUser, setCreateUser] = useState(false);
  const [showPass, setShowPass] = useState(false);

  const set = (k: keyof EmployeeFormData, v: string | number) =>
    setForm((f) => ({ ...f, [k]: v }));
  const setUser = (k: keyof UserExtraData, v: string) =>
    setUserExtra((u) => ({ ...u, [k]: v }));
    ...
    >
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto p-8 space-y-6 flex-1">
          ...
          {userError && (
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-amber-700 text-sm">
              <AlertCircle size={18} className="shrink-0 mt-0.5" />
              <div>
                <p className="font-bold">
                  تم إضافة الموظف بنجاح، لكن فشل إنشاء حساب المستخدم:
                </p>
                <p className="mt-0.5 text-amber-600">{userError}</p>
              </div>
            </div>
          )}

          {/* ── Dual-request toggle (add mode only) ── */}
          {mode === "add" && (
            <div
              onClick={() => setCreateUser((s) => !s)}
              className={`flex items-center justify-between gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 ${
                createUser
                  ? "bg-gradient-to-l from-[#1B5E4F]/5 to-[#0F4F3E]/5 border-[#1B5E4F]"
                  : "bg-gray-50 border-gray-200 hover:border-[#B8976B]/40"
              }`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${createUser ? "bg-[#1B5E4F]" : "bg-gray-200"}`}
                >
                  <Shield
                    className={createUser ? "text-white" : "text-gray-400"}
                    size={20}
                  />
                </div>
                <div>
                  <p
                    className={`text-sm font-bold transition-colors ${createUser ? "text-[#1B5E4F]" : "text-gray-600"}`}
                  >
                    إنشاء حساب مستخدم في النظام
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    سيتم إرسال طلبي إضافة موظف وتسجيل مستخدم معاً في نفس الوقت
                  </p>
                </div>
              </div>
              {createUser ? (
                <ToggleRight className="text-[#1B5E4F] shrink-0" size={32} />
              ) : (
                <ToggleLeft className="text-gray-300 shrink-0" size={32} />
              )}
            </div>
          )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => set("employeeFirstName", e.target.value)}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => set("employeeLastName", e.target.value)}
                />
              </div>
              <div>
                <label className={labelCls}>
                  البريد الإلكتروني
                  {createUser && (
                    <span className="text-[#1B5E4F] font-bold mr-1">
                      ← سيُستخدم للحساب
                    </span>
                  )}
                </label>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) =>
                      set(
                        "employmentType",
                        e.target.value as EmployeeFormData["employmentType"],
                      )
                    }
                  >
          ...
          {/* ── User account fields (shown only when toggle is ON) ── */}
          {createUser && mode === "add" && (
            <section className="rounded-2xl border-2 border-[#1B5E4F]/20 overflow-hidden">
              {/* Section header */}
              <div className="bg-gradient-to-l from-[#1B5E4F]/10 to-[#0F4F3E]/5 px-6 py-4 flex items-center gap-3 border-b border-[#1B5E4F]/10">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    البريد الإلكتروني سيُستخدم من أعلى تلقائياً
                  </p>
                </div>
              </div>
              <div className="p-6 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={userExtra.phoneNumber}
                    onChange={(e) => setUser("phoneNumber", e.target.value)}
                    dir="ltr"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={userExtra.nid}
                    onChange={(e) => setUser("nid", e.target.value)}
                    dir="ltr"
                  />
                </div>
                <div>
                  ...
                  <div className="relative">
                    <select
                      className={inputCls + " appearance-none"}
                      value={userExtra.gender}
                      onChange={(e) =>
                        setUser(
                          "gender",
                          e.target.value as UserExtraData["gender"],
                        )
                      }
                    >
                ...
                <div className="col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-[#B8976B] hover:text-[#1B5E4F] transition-colors"
                    >
        ...
        {/* Footer */}
        <div className="px-8 py-5 bg-gray-50 border-t border-[#B8976B]/10 flex gap-3 justify-between items-center shrink-0">
          {/* Left: dual-request indicator */}
          {mode === "add" && createUser && (
            <div className="flex items-center gap-2 text-[#1B5E4F] text-xs font-semibold">
              <div className="flex gap-1">
                <span className="w-2 h-2 rounded-full bg-[#1B5E4F] animate-pulse" />
                <span
                  className="w-2 h-2 rounded-full bg-[#B8976B] animate-pulse"
          ...
          <div className="flex gap-3 mr-auto">
            ...
            <button
              onClick={() => onSave(form, createUser ? userExtra : null)}
              disabled={saving}
...
const EmployeeCard = ({
  ...
  const initials =
    (employee.employeeFirstName?.[0] ?? "") +
    (employee.employeeLastName?.[0] ?? "");
      ...
      <div className="p-5 space-y-3 flex-1">
        <div className="flex items-center gap-2 text-[#4A4A4A]">
          ...
          <span className="text-sm truncate">
            {employee.sectorName ?? employee.sectorId}
          </span>
        </div>
        <div className="flex items-center gap-2 text-[#4A4A4A]">
        ...
        <div className="pt-1">
          <ContractBadge type={employee.employmentType} />
        </div>
      </div>
...
const EmployeesPage = () => {
  ...
  const [userCreateErr, setUserCreateErr] = useState<string | null>(null);
  ...
  const closeModal = () => {
    ...
    setSelected(null);
    setFormError(null);
    setUserCreateErr(null);
  };

  // ── ADD (handles dual-request) ────────────────────────────────────────────
  const addMutation = useMutation({
    ...
    onMutate: async ({ emp }) => {
      ...
      const temp: Employee = {
        ...
        sectorName: sectors.find((s) => s.id === emp.sectorId)?.name,
      };
    ...
    onSuccess: (result) => {
      // Show amber warning if employee was created but user registration failed
      if (result.userError) {
        setUserCreateErr(result.userError);
        setModal("add"); // re-open with partial failure message
      }
    },
  ...
  const handleSave = (
    ...
    setFormError(null);
    setUserCreateErr(null);
    if (modal === "add") addMutation.mutate({ emp, userExtra });
    else if (modal === "edit" && selected)
      editMutation.mutate({ id: selected.id, data: emp });
  };
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-br from-[#1B5E4F] to-[#0F4F3E] p-6 relative">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#B8976B] to-[#9A7D5B] flex items-center justify-center text-white font-bold text-2xl">
                  {selected.employeeFirstName[0]}
                  {selected.employeeLastName[0]}
                </div>
                <div>
                  <h2 className="text-xl font-bold text-white">
            ...
            <div className="p-6 grid grid-cols-2 gap-4 text-sm">
              {[
                ...
                {
                  label: "القطاع",
                  value: selected.sectorName ?? selected.sectorId,
                },
                { label: "الموقع", value: selected.location },
                {
                  label: "الراتب",
                  value: `${selected.baseSalary?.toLocaleString("ar-SA")} ريال`,
                },
                {
                  label: "تاريخ التعيين",
                  value: new Date(selected.hireDate).toLocaleDateString(
                    "ar-SA",
                  ),
                },
                {
